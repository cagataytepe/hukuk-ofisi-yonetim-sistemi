-- Move task soft delete into an explicit, permission-checked RPC.
...
begin
  ...
  if not coalesce(v_has_permission, false) then
    raise exception 'Gorev silme yetkiniz yok.'
      using errcode = '42501';
  end if;
  ...
  return query
    update public.tasks t
     ...
     returning t.id, t.deleted_at;

  get diagnostics v_updated_count = row_count;

  if v_updated_count = 0 then
    raise exception 'Gorev kaydi bulunamadi veya zaten silinmis.'
      using errcode = 'P0002';
  end if;
end;
$$;
