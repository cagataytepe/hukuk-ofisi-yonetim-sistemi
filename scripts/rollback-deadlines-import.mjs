...
const reportPath = args.values.report || `work/deadlines-rollback-${batchId || "preview"}.json`;
...
const { data: updatedRows, error: updatedError } = await supabase
  .from("deadlines")
  .select("id, legacy_id, import_batch_id, metadata, deleted_at")
  .eq("metadata->>lastDeadlinesImportBatchId", batchId)
  .is("deleted_at", null)
...
if (!confirm) {
  ...
  console.log("Sureli isler rollback onizleme tamamlandi. Veri degistirilmedi.");
...
writeJsonReport(reportPath, report);
console.log("Sureli isler rollback tamamlandi.");
console.log(`Batch ID: ${batchId}`);
